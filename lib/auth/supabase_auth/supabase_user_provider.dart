...
class BaghdadPaySupabaseUser extends BaseAuthUser {
  BaghdadPaySupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> baghdadPaySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BaghdadPaySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
